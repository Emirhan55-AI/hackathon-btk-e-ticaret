...
# =============================================================================
# FEEDBACK LOOP ORCHESTRATION ENDPOINTS
# =============================================================================

class FeedbackRequest(BaseModel):
    """Request model for user feedback processing."""
    user_id: str = Field(..., description="User identifier")
    recommendation_id: str = Field(..., description="Recommendation being reviewed")
    feedback_text: str = Field(..., description="User's feedback text")
    context: Optional[Dict[str, Any]] = Field(default={}, description="Additional context")

class FeedbackInsightsRequest(BaseModel):
    """Request model for feedback insights."""
...
async def process_user_feedback(request: FeedbackRequest):
    """
    Process user feedback through intelligent prompt engineering patterns.
    Orchestrates feedback analysis and triggers appropriate improvement actions.
    """
    try:
        ...
        # Process feedback through orchestrated workflow
        result = await orchestrator.process_user_feedback(feedback_data)
        
        if result['success']:
            logger.info(f"✅ Feedback processed successfully: {result['workflow_id']}")
            return {
                "success": True,
                "message": "Feedback processed and learning applied",
            ...
            logger.error(f"❌ Feedback processing failed: {result.get('error')}")
            raise HTTPException(
                status_code=500,
...
async def get_feedback_insights(request: FeedbackInsightsRequest):
    """
    Get comprehensive feedback insights and learning analytics.
    Provides intelligence about user preferences and system improvement trends.
    """
    try:
        logger.info(f"📊 Getting feedback insights for user: {request.user_id or 'all users'}")
        
        if not orchestrator:
            raise HTTPException(status_code=503, detail="Orchestrator not initialized")
        
        # Get insights through orchestrator
        result = await orchestrator.get_feedback_insights(
            user_id=request.user_id,
            time_range=request.time_range
        )
        
        if result['success']:
            logger.info("✅ Feedback insights retrieved successfully")
            return {
                "success": True,
            ...
            logger.error(f"❌ Failed to get feedback insights: {result.get('error')}")
            raise HTTPException(
                status_code=500,
                detail=f"Failed to get insights: {result.get('error')}"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Insights retrieval error: {str(e)}"
        )

@app.get("/feedback/analytics")
async def get_feedback_analytics():
    """
    Get comprehensive feedback system analytics.
    Provides system-wide metrics and performance indicators.
    """
    try:
        logger.info("📈 Getting feedback system analytics")
        
        if not orchestrator:
            raise HTTPException(status_code=503, detail="Orchestrator not initialized")
        
        # Get orchestration analytics
        orchestration_analytics = orchestrator.get_orchestration_analytics()
        
        # Get feedback-specific analytics
        feedback_insights = await orchestrator.get_feedback_insights()
        
        return {
            "success": True,
            "analytics": {
                "orchestration": orchestration_analytics,
                "feedback_system": feedback_insights.get('insights', {}),
                "feedback_processing": {
                    "total_feedback_workflows": len([w for w in orchestrator.completed_workflows 
                                                   if any('feedback' in step.step_id for step in w.steps)]),
                    "average_processing_time": orchestrator._calculate_avg_feedback_time(),
                    "coordination_success_rate": orchestrator._calculate_coordination_success_rate(),
                }
            },
            "timestamp": datetime.now().isoformat()
        }
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Analytics retrieval error: {str(e)}"
        )
